...
namespace joint_impedance_controller
{
  ...
  ctrl::VectorND JointImpedanceController::computeTorque()
  {
    ...
    if (m_compensate_coriolis)
    {
      ...
      // RCLCPP_INFO_STREAM(get_node()->get_logger(), "tau coriolis: " << tau_coriolis.data.transpose());
    }

    // RCLCPP_INFO_STREAM(get_node()->get_logger(), "tau control: " << (tau + tau_gravity.data).transpose());
