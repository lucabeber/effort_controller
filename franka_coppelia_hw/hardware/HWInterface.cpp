...
#include "franka_coppelia_hw/HWInterface.hpp"
...
FrankaEffortHardware::on_init(const hardware_interface::HardwareInfo &info) {
  if (hardware_interface::SystemInterface::on_init(info) !=
      hardware_interface::CallbackReturn::SUCCESS) {
    return hardware_interface::CallbackReturn::ERROR;
  }
  // initializing variable to save the current joint state
  current_joint_state.name = {"panda_joint1", "panda_joint2", "panda_joint3",
  ...
  for (const hardware_interface::ComponentInfo &joint : info_.joints) {
    // RRBotSystemPositionOnly has exactly two states and command interface on
    // each joint
    if (joint.command_interfaces.size() != 1) {
      RCLCPP_FATAL(rclcpp::get_logger("FrankaEffortHardware"),
...
hardware_interface::CallbackReturn FrankaEffortHardware::on_configure(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_INFO(rclcpp::get_logger("FrankaEffortHardware"),
  ...
  float initial_conf[] = {
      0.0, 2.0, 0.0, -90.0 * 3.14159 / 180.0, 0.0, 90.0 * 3.14159 / 180.0, 0.0};
...
FrankaEffortHardware::export_state_interfaces() {
...
FrankaEffortHardware::export_command_interfaces() {
...
hardware_interface::CallbackReturn FrankaEffortHardware::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_INFO(rclcpp::get_logger("FrankaEffortHardware"),
  ...
  RCLCPP_INFO(rclcpp::get_logger("FrankaEffortHardware"),
              "HW Interface successfully activated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn FrankaEffortHardware::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  // BEGIN: This part here is for exemplary purposes - Please do not copy to
  // your production code
  RCLCPP_INFO(rclcpp::get_logger("FrankaEffortHardware"),
...
FrankaEffortHardware::read(const rclcpp::Time & /*time*/,
  ...
  for (uint i = 0; i < hw_pos_.size(); i++) {
    ...
    // RCLCPP_INFO_STREAM(rclcpp::get_logger("FrankaEffortHardware"), "pos " <<
    // current_joint_state.position[i]);
    // RCLCPP_INFO_STREAM(rclcpp::get_logger("FrankaEffortHardware"), "vel " <<
...
hardware_interface::return_type
FrankaEffortHardware::write(const rclcpp::Time & /*time*/,
                            const rclcpp::Duration & /*period*/) {
...
PLUGINLIB_EXPORT_CLASS(franka_coppelia_hw::FrankaEffortHardware,
